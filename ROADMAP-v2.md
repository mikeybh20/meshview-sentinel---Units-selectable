...
- ✅ **`/api/mesh/connect/tcp` desyncs `meshBridge` from BridgeManager.** Fixed in commit `432d555`. New `bridgeManager.rebindSingletonTcp(host, port)` handles the disconnect/reset-latches/reconnect dance so contexts stay coherent after a TCP rebind. Closes the root cause of the singleton-detached bug class.
- ✅ **`meshBridge.connected` guards in ~20 admin-write endpoints.** Fixed in commit `aac6351`. New `resolveBridge(req)` / `defaultDbBridge()` / `forEachBridge(fn)` helpers route every module-config / group / waypoint / retention write through BridgeManager. Module endpoints now accept `?radio_id=<short>` so operators can configure a SECONDARY radio (e.g., refresh telemetry on WRTJ) — previously those silently no-op'd against the wrong bridge.

### Features

- ✅ **mDNS / Bonjour auto-discovery of TCP radios.** Shipped in Beta 3 via `bonjour-service` — commit `a72d690`.
- ✅ **Per-radio "Active" lens.** Commit `4b048e9`. New `Heard ever / Active · 60m` toggle visible when a specific radio is selected. "Active" gates on `lastHeardAtPerRadio[selectedRadio]` within the last 60 min — cuts through the MQTT-bridged heard-by overlap to show what's actually reaching that radio right now.

### Polish (carried from operator UX feedback)

- ✅ **NetworkConfig write over local admin.** Shipped in `52b4123`. New `bridge.setNetworkConfig()` + `POST /api/mesh/radios/:radioId/network` + editable form in `NetworkConfigSection`. Strictly local-admin: API refuses if the radio has no live BridgeManager context, its transport isn't serial/tcp, or it's disconnected — the bridge's `sendAdminMessage` already writes only over the operator's own connection (never LoRa-relayed). PSK never echoed in readbacks → form re-asks on every save with confirmation copy explaining the trade-off. Logs redact the PSK.
- ✅ **CSV import stub removed.** Commit `4b048e9`. The button, `ImportModal.tsx`, and its prop/state in `SettingsModal` + `App.tsx` are gone. Migration is properly covered by the encrypted Full Backup (`b21659d`, `b1ee5b0`); BBS archival is covered by plaintext CSV exports (`3614cc8`).
