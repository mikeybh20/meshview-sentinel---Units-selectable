...
- 🔲 **`/api/mesh/connect/tcp` desyncs `meshBridge` from BridgeManager.**
  Root cause behind a whole class of "the snapshot shows X but per-radio endpoint shows Y" bugs we patched in Beta 3. The legacy endpoint mutates `meshBridge.transport` directly without going through BridgeManager, so `contexts[defaultRadioId].bridge` ends up out of step with `meshBridge`'s actual state. We Band-Aided the read side by routing `/api/mesh/snapshot`, `/api/mesh/status`, the favorite handler, etc. through `bridgeManager.getDefault()?.bridge`, but the underlying desync is still possible. Beta 4 fix: deprecate the bare `/api/mesh/connect/tcp` endpoint and route operators through `bridgeManager.promoteToSingleton()` instead (which already correctly tears down + rebinds and keeps contexts coherent). See commits `f3f14eb`, `6409a95`, `36b1a86`, `c09fee4` for the Band-Aid history.
- 🔲 **`meshBridge.connected` guards in ~20 admin-write endpoints.**
  Same singleton-detached bug class as the snapshot fix above, but on the write side. Every `if (!meshBridge.connected) return 503` check (set-channel, refresh-config, network-config, position, display, bluetooth, canned-messages, etc.) reads from the singleton, which can be detached. The fix is to accept `?radio_id=<short>` on each endpoint and route the write through `bridgeManager.get(radioId).bridge`. Tedious but mechanical — about 20 sites in `server/api.ts`. Will fix the "I can't save channels to WRTJ" / "WiFi config refuses to write to the secondary" symptoms operators will hit.

### 🔲 Features

- ✅ **mDNS / Bonjour auto-discovery of TCP radios.** Shipped in Beta 3 via `bonjour-service` — see commit `a72d690`.
- 🔲 **Per-radio LoRa-only node view.** Today the per-radio filter on the Dashboard shows nodes by `heardByRadios.includes(selectedRadioId)`. Both radios subscribe to MQTT, so most operator setups end up with `heardByRadios = [radioA, radioB]` for every node and the filter looks like it isn't working. Add a "LoRa-recent" lens that filters using `lastHeardAtPerRadio[radioId]` AND `lastVia === 'lora'` AND last-heard within the last N minutes — a real per-radio distinction that operators can actually see.
...
- 🔲 **CSV import is a stub.** [`ImportModal.tsx:34`](src/components/ImportModal.tsx#L34) literally has `// In a real app, logic would map CSV fields to state` — clicking "Import" parses your CSV, says "Successfully parsed N records," and then throws the result away. Either implement actual bulk-load semantics (most useful: node alias / favorite import) or remove the button until it's real. Misleading UI as-is.
- 🔲 **CSV export only sees what's in the current React snapshot.** [`ExportModal.tsx`](src/components/ExportModal.tsx) filters from `props.messages` / `props.events` — which is what the dashboard has currently loaded. A date range from a month ago produces an empty CSV even when those rows exist in the DB. Plan: server-side `/api/mesh/export/csv?type=<>&radio_id=<>&from=<ts>&to=<ts>` endpoint that streams from SQLite directly, so exports cover the full retention window.
- 🔲 **BBS subscriber / mail CSV export.** The full backup (commit `b21659d`) captures BBS mail + weather subscribers, but there's no plaintext CSV export for them. Operators have asked for this for compliance / archival. Trivial server-side query, gated by the same radio_id filter as the mail / weather views.
- 🔲 **Selective restore.** Today restore is all-or-nothing per envelope. A "restore radios only" / "restore BBS mail only" UI would let operators share channel configs between installs without clobbering the rest of the target's state.
- 🔲 **Schema-drift recovery in restore.** `loadTable` today emits a warning + skips the row when a backup column doesn't exist in the current schema. Pre-filter to current-schema columns at restore time so future schema additions don't leave operators staring at warnings.
