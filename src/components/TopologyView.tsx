...
export function TopologyView({ nodes, onNodeSelect }: { nodes: MeshNode[], onNodeSelect: (id: string) => void }) {
  ...
  const graphData = React.useMemo(() => {
    ...
    if (isFocusMode && focusedNodeId) {
      ...
      // If focused node is homeBase, neighbors are all online nodes
      if (focusedNodeId === '!abcdef01') {
        nodes.forEach(n => n.online && neighbors.add(n.id));
      } else if (homeBase) {
        neighbors.add(homeBase.id); // Every node connects to homeBase in our sim
      }
  ...
  }, [nodes, focusedNodeId, isFocusMode]);
  ...
  React.useEffect(() => {
    ...
    const link = container.append('g')
      ...
      .attr('stroke', (d: any) => {
        if (d.rssi > -70) return '#10b981'; // emerald
        if (d.rssi > -90) return '#f59e0b'; // amber
        return '#ef4444'; // red
      })
  ...
  React.useEffect(() => {
    ...
    nodeSel.select('.glow-ring')
      .attr('r', (d: any) => d.isHomeBase ? 18 : (d.id === activeId ? 14 : 10));
